...
#elif defined(AIRQ4_STEREO)
const juce::String AirQ4AudioProcessor::getName() const {
  return "AirQ4 Stereo";
}
#else
...
double AirQ4AudioProcessor::getTailLengthSeconds() const { return 0.0; }
...
int AirQ4AudioProcessor::getCurrentProgram() { return 0; }
...
void AirQ4AudioProcessor::changeProgramName(int index,
                                            const juce::String &newName) {
  juce::ignoreUnused(index, newName);
}

void AirQ4AudioProcessor::prepareToPlay(double sampleRate,
                                        int samplesPerBlock) {
  ...
  subLowCut25HzFilter.coefficients =
      juce::dsp::IIR::Coefficients<float>::makeHighPass(
          sampleRate, SUB_LOW_CUT_FREQ, SUB_Q_MIN);
  lowShelf40HzFilter.coefficients =
      juce::dsp::IIR::Coefficients<float>::makeLowShelf(
          sampleRate, LOWSHELF_FREQ, LOWSHELF_Q_MIN,
          juce::Decibels::decibelsToGain(0.0f));
  bell160HzFilter.coefficients =
      juce::dsp::IIR::Coefficients<float>::makePeakFilter(
          sampleRate, BELL1_FREQ, BELL_Q_MIN,
          juce::Decibels::decibelsToGain(0.0f));
  bell650HzFilter.coefficients =
      juce::dsp::IIR::Coefficients<float>::makePeakFilter(
          sampleRate, BELL2_FREQ, BELL_Q_MIN,
          juce::Decibels::decibelsToGain(0.0f));
  bell2k5HzFilter.coefficients =
      juce::dsp::IIR::Coefficients<float>::makePeakFilter(
          sampleRate, BELL3_FREQ, BELL_Q_MIN,
          juce::Decibels::decibelsToGain(0.0f));
  highShelfAirFilter.coefficients =
...
bool AirQ4AudioProcessor::isBusesLayoutSupported(
...
void AirQ4AudioProcessor::processBlock(juce::AudioBuffer<float> &buffer,
  ...
  subLowCut25HzFilter.coefficients =
      juce::dsp::IIR::Coefficients<float>::makeHighPass(getSampleRate(),
                                                        SUB_LOW_CUT_FREQ, Qsub);

  auto [gainLS, qLS] = calcLowShelf40Hz(sliderLowShelf40Hz);
  lowShelf40HzFilter.coefficients =
      juce::dsp::IIR::Coefficients<float>::makeLowShelf(
          getSampleRate(), LOWSHELF_FREQ, qLS,
          juce::Decibels::decibelsToGain(gainLS));

  auto [gain160, q160] = calcBell(sliderBell160Hz);
  bell160HzFilter.coefficients =
      juce::dsp::IIR::Coefficients<float>::makePeakFilter(
          getSampleRate(), BELL1_FREQ, q160,
          juce::Decibels::decibelsToGain(gain160));

  auto [gain650, q650] = calcBell(sliderBell650Hz);
  bell650HzFilter.coefficients =
      juce::dsp::IIR::Coefficients<float>::makePeakFilter(
          getSampleRate(), BELL2_FREQ, q650,
          juce::Decibels::decibelsToGain(gain650));

  auto [gain2k5, q2k5] = calcBell(sliderBell2k5Hz);
  bell2k5HzFilter.coefficients =
      juce::dsp::IIR::Coefficients<float>::makePeakFilter(
          getSampleRate(), BELL3_FREQ, q2k5,
          juce::Decibels::decibelsToGain(gain2k5));
  ...
  highShelfAirFilter.coefficients =
      juce::dsp::IIR::Coefficients<float>::makeHighShelf(
          getSampleRate(), selectedAirFreq, qHS,
          juce::Decibels::decibelsToGain(gainHS));
...
void AirQ4AudioProcessor::setStateInformation(const void *data,
                                              int sizeInBytes) {
